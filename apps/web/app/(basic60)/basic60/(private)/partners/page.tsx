...
export default async function Basic60PartnersPage({ searchParams }: { searchParams: ApprovedBasic60SearchParams }) {
  const { locale, countryCode } = await approvedBasic60PageContext(searchParams);
  return <ApprovedBasic60ModulePage kind="partners" locale={locale} countryCode={countryCode} basePath="/basic60" />;
}
