...
登录前后均可使用顶部`中 / EN`切换；默认中文，选择写入`navigator_locale` Cookie，
刷新后保留，并在不改变URL的前提下同步页面文案、接口内容、日期和数字格式。
...
- 以低密度双语落地页说明“新能源企业出海工作台”价值，并按首页、全球市场、
  政策与风险、出海工具和合作伙伴组织一级导航；
- 使用仓库内置世界国界轮廓和本地WebGL动态地球，支持旋转、拖动、国家面悬停高亮、
  滚轮/触控缩放、按钮缩放和全屏式放大；不在地球下方单列首期五国，五个合成市场
  通过单一选择器定位，WebGL
  不可用时回退到可键盘操作的选择器；运行时不请求外部地图、瓦片或纹理；
- 完成国家浏览、国家详情、政策、风险、机会和伙伴只读演示链路；
- 在`/tools`提供AI出海助手、光储方案、可研报告草案和项目招标四项确定性演示流程；
- `/compare`保留为严格双国辅助比较，正式2—4国比较合同保持不变；原`/tenders`
  路径重定向到`/tools/tenders`；
- 全站支持`zh-CN`与`en`，中英文界面、五国和合成业务记录均由仓库内词典及fixture
  提供，不调用在线翻译、外部字体或外部模型；
- 通过共享演示口令与同源服务端代理隔离后端密钥，所有页面和响应持续标记
  `synthetic_demo`及当前语言的“演示数据 / 非正式结论”提示；
