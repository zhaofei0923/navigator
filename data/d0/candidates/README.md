...
| `d0_baseline_adoption_review_bundle.2026-08-02.json` | 权威源、候选、整改包、AC-001/002批准与变更规模的哈希绑定交接包 | 仅供项目批准人决定是否进入人工基线采用；不会改写或激活权威工作簿 |
...
4. 校验通过后运行`prepare-d0-baseline-change --resolution <整改副本> --output
   ...
   公式改动；
5. 候选工作簿验证且AC-001/002验收通过后，运行`prepare-d0-baseline-review`生成项目
   批准人可签的精确哈希交接包；该命令只准备审核材料，不会替换或批准权威基线；
6. 项目批准人批准交接包后仍须由正式基线责任流程发布权威修订版，并同步更新Markdown、
   技术附件、D0台账和版本记录；
