...
2. 复制`core_contract_resolution.template.json`，逐项填写实名提案人与复核人、变更单、
   时间和证据，再运行`validate-d0-contract-resolution --input <副本>`；不得删减项目；
3. 校验通过后在独立候选D0工作簿中实施，再运行
   `validate-d0-baseline-change --resolution <整改副本> --workbook <候选工作簿>`；验证器
   要求原字段顺序不变，只追加已评审主键字段和`单位`列，拒绝其他单元格或公式改动；
4. 候选工作簿验证通过仍须接受正式变更评审，不会替换或批准权威基线；
5. 正式变更必须同步更新Markdown、技术附件、D0台账和版本记录；
6. 真实金标准必须由合规和专业负责人补齐最终结论及各自证据；
7. 机器状态为`fail`时不得批准；`pass`也只表示可进入实名人工复核；
8. 审批证据登记到 `data/d0/evidence/manifest.json`，不得直接编辑机器结论伪造通过；
9. 更新正式基线后重新运行 `snapshot`、`prepare-d0`、`validate`和`gate --stage D0`。
