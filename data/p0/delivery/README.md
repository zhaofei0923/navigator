...
5. 填写发布指标；再逐项完成构建、部署、回滚、恢复和移交`release_capabilities`，
   记录最终提交、执行人/时间、授权复核人/时间和证据编号。每项能力的`commit_sha`
   以及指标的`evaluated_commit_sha`都必须等于最终发布的精确提交。最终发布批准人
   必须是D4项目委员会的唯一实名签署人。所有`*_at`字段必须是`YYYY-MM-DD`或带时区
   的ISO-8601时间；能力复核和测试复核不得早于执行，证据批准不得早于生成，最终
   发布批准不得早于任何实现、授权、测试复核、ACC验收、发布能力执行/复核、指标
   测量或证据批准事件。
