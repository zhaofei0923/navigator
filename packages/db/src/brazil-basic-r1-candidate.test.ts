...
const RUN_ID = "data-basic-br-20260717-r1";
...
const SOURCE_BINDINGS = [
  { sourceId: "brazil-epe-ben-2026-summary", sourceName: "Empresa de Pesquisa Energetica (EPE)", sourceUrl: "https://www.epe.gov.br/pt/imprensa/noticias/epe-publica-o-relatorio-sintese-do-balanco-energetico-nacional-2026", retrievedAt: "2026-07-17T16:29:48.257Z", publishedAt: "2026-06-03T00:00:00.000Z", contentSha256: "5d07ea1f1194806d1c84390b501aafc54b509f297e70410f788d180a8e4a7ccd", sourceFamily: "energy-authority" },
  { sourceId: "brazil-ipea-ods7-renewable-target", sourceName: "Institute for Applied Economic Research (IPEA)", sourceUrl: "https://www.ipea.gov.br/ods/ods7.html", retrievedAt: "2026-07-17T16:29:51.102Z", publishedAt: null, contentSha256: "1236c59d4dc783d8d145645a661fbf9aafb9e0643298f1e1374b1b5225e42569", sourceFamily: "government" },
  { sourceId: "world-bank-country", sourceName: "World Bank", sourceUrl: "https://api.worldbank.org/v2/country/BR?format=json", retrievedAt: "2026-07-17T16:29:52.069Z", publishedAt: null, contentSha256: "9ba15dd0206c394794295ee818cf986c12a513a306723785cc845ea8aeaa1669", sourceFamily: "international-organization" },
  { sourceId: "world-bank-gdp", sourceName: "World Bank", sourceUrl: "https://api.worldbank.org/v2/country/BR/indicator/NY.GDP.MKTP.CD?source=2&format=json&mrv=1&per_page=1", retrievedAt: "2026-07-17T16:30:04.400Z", publishedAt: null, contentSha256: "1e7e6d6c24d45edea5ad6c7d51ca1b55b1e731a13f3048eecdd6a67698368534", sourceFamily: "international-organization" },
  { sourceId: "world-bank-gdp-growth", sourceName: "World Bank", sourceUrl: "https://api.worldbank.org/v2/country/BR/indicator/NY.GDP.MKTP.KD.ZG?source=2&format=json&mrv=1&per_page=1", retrievedAt: "2026-07-17T16:30:11.156Z", publishedAt: null, contentSha256: "737fe896e4c99bea79476a144828fb99f20ab974ce446501b989c9dd9d4fa04d", sourceFamily: "international-organization" },
  { sourceId: "world-bank-population", sourceName: "World Bank", sourceUrl: "https://api.worldbank.org/v2/country/BR/indicator/SP.POP.TOTL?source=2&format=json&mrv=1&per_page=1", retrievedAt: "2026-07-17T16:30:14.346Z", publishedAt: null, contentSha256: "df53cd21ccfcaff8d86b226e32a32161cd44e1a73008d6f07c4d735d46fab1ba", sourceFamily: "international-organization" },
] as const;
...
const EXPECTED_EVIDENCE_LOCATORS = {
  ...
  "marketOverview.industryTags": ["brazil-epe-ben-2026-summary|html:article-body#solar-pv-and-wind-2025"],
...
const INDICATORS = [
  {
    label: { zh: "最终电力消费同比增长", en: "Final electricity consumption year-on-year growth" },
    value: "2.7",
    unit: "%",
    year: 2025,
  },
  {
    label: { zh: "太阳能光伏装机容量", en: "Solar PV installed capacity" },
    value: "64793",
    unit: "MW",
    year: 2025,
  },
  {
    label: { zh: "风电装机容量", en: "Wind installed capacity" },
    value: "34707",
    unit: "MW",
    year: 2025,
  },
...
describe("Brazil Basic r1 candidate", () => {
  ...
  test("locks the reviewed draft-only audit bundle", () => {
    ...
    if (bundle.sourceRegister.schemaVersion !== BASIC_COLLECTION_AUDIT_V2_SCHEMA_VERSION) {
      throw new Error("expected Brazil r1 candidate to use the v2 audit contract");
    }
    ...
    expect(factsByPath.get("marketOverview.industryTags")).toMatchObject({
      ...
      evidence: [{
        ...
        locator: "html:article-body#solar-pv-and-wind-2025",
    ...
    expect(sourceLocators.get("brazil-epe-ben-2026-summary")?.has(EMPTY_TECH_TAGS_LOCATOR))
      .toBe(true);
    expect(JSON.stringify(bundle.extractedFacts)).not.toContain("20.4");
    expect(JSON.stringify(bundle.extractedFacts)).not.toMatch(/86\\.8|86\\.6/);
    expect(bundle.marketOverviewDraft.renewableTarget).toEqual(RENEWABLE_TARGET);
    expect(bundle.marketOverviewDraft.renewableTarget.en).toContain(
      "not specific to the electricity matrix",
    );
    expect(bundle.marketOverviewDraft.renewableTarget.en).not.toMatch(/legal obligation|[0-9]+%/i);
    ...
    expect(readdirSync(stagingDirectory).sort(compareText)).toEqual(Object.keys(ARTIFACT_HASHES));
