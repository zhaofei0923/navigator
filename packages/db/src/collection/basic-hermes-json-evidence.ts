...
function snapshotFact(value: unknown): BasicExtractedFact {
  ...
  if (!isPath(fieldPath) || fact.get("factId") !== factId(fieldPath) || !includes(["candidate", "conflict"] as const, status) || fact.get("extractionMethod") !== "deterministic" || !(fact.get("uncertainty") === null || nonblank(fact.get("uncertainty")))) invalid();
  ...
  return { factId: fact.get("factId") as string, fieldPath, status, evidence: parsed, extractionMethod: "deterministic", uncertainty: fact.get("uncertainty") as string | null };
}
