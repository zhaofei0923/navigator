...
import { factId, materializeBasicHermesFacts, type BasicHermesSourcedObservation } from "./basic-hermes-evidence-materializer.js";
import {
...
const CAPTURE = ["sourceId", "contentSha256", "byteLength", "reused", "body", "finalUrl", "redirectChain", "contentType", "retrievedAt"] as const;
...
function snapshotCapture(value: unknown, policy: BasicHermesSourcePolicy): { receipt: BasicRawCaptureReceipt; body: Uint8Array; retrievedAt: string; contentSha256: string } {
  const capture = record(value, CAPTURE); if (capture === null) captureInvalid();
  const redirectChain = values(capture.get("redirectChain"), 0);
  if (capture.get("sourceId") !== policy.sourceId || typeof capture.get("reused") !== "boolean" || redirectChain === null || !timestamp(capture.get("retrievedAt")) || !isBasicJsonContentType(capture.get("contentType"))) captureInvalid();
  const finalUrl = captureUrl(capture.get("finalUrl")); if (finalUrl !== policy.sourceUrl || !allowedUrl(finalUrl, policy)) captureInvalid();
...
function snapshotObservations(value: unknown, raw: BasicCollectionJsonValue, sourceId: string, policy: BasicHermesSourcePolicy): BasicHermesSourcedObservation[] {
  ...
  return observations.map((value) => { const observation = record(value, OBSERVATION); if (observation === null) invalid(); const fieldPath = text(observation.get("fieldPath")); const locator = text(observation.get("locator")); const rawValue = json(observation.get("rawValue")); const capturedRaw = pointer(raw, locator); const normalizedValue = json(observation.get("normalizedValue")); const unit = observation.get("unit"); const year = observation.get("year"); const uncertainty = observation.get("uncertainty"); if (!isPath(fieldPath) || !(unit === null || nonblank(unit)) || !(year === null || finite(year)) || !(uncertainty === null || nonblank(uncertainty)) || !sameJson(rawValue, capturedRaw)) invalid(); return { sourceId, fieldPath, locator, rawValue: json(capturedRaw), normalizedValue, unit: unit as string | null, year: year as number | null, uncertainty: uncertainty === null ? null : (uncertainty as string).trim(), untrusted: policy.accessStatus !== "open" || policy.credibility === "UNVERIFIED" || policy.promptInjectionRisk !== "none" }; });
}
...
function snapshotSource(value: unknown): BasicSourceRecord {
  const source = record(value, SOURCE); if (source === null) invalid(); const sourceId = text(source.get("sourceId")); const sourceUrl = httpsUrl(source.get("sourceUrl")); const locators = strings(source.get("evidenceLocators"), 1);
  if (!SOURCE_ID.test(sourceId) || !timestamp(source.get("retrievedAt")) || !(source.get("publishedAt") === null || timestamp(source.get("publishedAt"))) || typeof source.get("contentSha256") !== "string" || !SHA256.test(source.get("contentSha256") as string) || !includes(FAMILIES, source.get("sourceFamily")) || source.get("accessStatus") !== "open" || !(source.get("accessNotes") === null || nonblank(source.get("accessNotes"))) || !includes(CREDIBILITIES, source.get("credibility")) || source.get("discoveryOnly") !== false || !includes(["none", "suspected", "confirmed"] as const, source.get("promptInjectionRisk"))) invalid();
...
function snapshotFact(value: unknown): BasicExtractedFact {
  const fact = record(value, FACT); if (fact === null) invalid(); const evidence = values(fact.get("evidence"), Number.MAX_SAFE_INTEGER); if (evidence === null) invalid(); const status = fact.get("status"); const fieldPath = text(fact.get("fieldPath")); const uncertainty = fact.get("uncertainty");
  if (!isPath(fieldPath) || fact.get("factId") !== factId(fieldPath) || !includes(["candidate", "conflict"] as const, status) || fact.get("extractionMethod") !== "deterministic" || !(uncertainty === null || nonblank(uncertainty) && uncertainty === uncertainty.trim())) invalid();
  ...
  if (status === "candidate" && parsed.length === 0 || status === "conflict" && new Set(parsed.map((item) => item.sourceId)).size < 2) invalid();
  return { factId: fact.get("factId") as string, fieldPath, status, evidence: parsed, extractionMethod: "deterministic", uncertainty: uncertainty as string | null };
}
...
function strings(value: unknown, min: number): string[] { const result = values(value, Number.MAX_SAFE_INTEGER); if (result === null || result.length < min || result.some((item) => !nonblank(item)) || !unique(result as string[])) invalid(); return result as string[]; }
...
function safeUrl(value: unknown): string { const raw = text(value); if (raw !== raw.trim()) invalid(); const url = canonicalBasicHermesDiscoveryUrl(raw); if (url === null || url === "forbidden") invalid(); return raw; }
function captureUrl(value: unknown): string { try { return safeUrl(value); } catch { captureInvalid(); } }
function httpsUrl(value: unknown): string { const raw = text(value); try { const url = new URL(raw); if (raw !== raw.trim() || url.protocol !== "https:" || url.username !== "" || url.password !== "") invalid(); return raw; } catch { invalid(); } }
...
function text(value: unknown): string { if (!nonblank(value)) invalid(); return value; }
