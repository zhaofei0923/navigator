...
- Linux with `/proc/self/fd`, `openat`, `mkdirat`, `fstatat`, and `renameat2(RENAME_NOREPLACE)`.
...
There is no JavaScript, copy, ordinary rename, cross-device, child-process, or non-native runtime fallback. Filesystems that do not support `RENAME_NOREPLACE`, including observed WSL DrvFS cases returning `EINVAL`, fail closed and may retain a private mode-`0700` orphan for inspection.
