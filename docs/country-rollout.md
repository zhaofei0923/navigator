...
| 扩展 | SA | 沙特阿拉伯 | Basic | r1 在批准前已 superseded；仅 r2 可待人工审核，且仅在独立发布决定获批后才能进入单独发布任务 | r2 候选待人工审核（未发布） |
...
- 下一步只能是针对 r2 的独立人工决定；只有该决定获批后，才可创建并执行单独的 `DATA-BASIC-SA-PUBLISH` 任务。r1 永不发布，r2 也不授权 Standard、Complete 或 AI 启用。
