...
| 覆盖等级 | 目标规模 | 说明 |
|----------|----------|------|
| Complete | 0 个（当前） | 未来经单独人工批准的升级目标 |
| Standard | 0 个（当前） | 未来经单独人工批准的升级目标 |
| Basic | 30–50 个 | 所有选定国家的必经入口，建立统一入口与基础画像 |

所有国家使用同一套 10 模块结构，差异只体现在数据深度，不体现在页面结构。

所有选定国家（包括 `ID`）必须先完成其两位国家码的 `DATA-BASIC-<ISO2>` 任务卡并达到 Basic；印尼使用 `DATA-BASIC-ID`。当前没有国家被指定为 Standard 或 Complete；只有在 Basic 之后，才可由人工批准单独的 Standard 或 Complete 升级任务。禁止国家直接以 Standard 或 Complete 进入产品。
...
|------|--------|------|----------|------|------|
| 首批 | ID | 印度尼西亚 | Basic | 通过 `DATA-BASIC-ID` 建立国家基础画像与统一模块占位 | 已批准迁移 |
...
`ID`、`VN`、`SA`、`AE`、`BR` 均必须先按 Basic 交付，再在独立任务卡中由人工决定是否推进 Standard 或 Complete。首批 30–50 个 Basic 国家清单、最终国家顺序和覆盖升级结论均属于业务优先级决策，必须由人工确认后再进入 seed 或后台录入。
