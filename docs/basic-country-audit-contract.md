...
`countryDirectory` 是与 `data/<country>/` 一致的国家目录名；`runId` 是稳定的批次标识。所有四个产物的 `runId`、`countryCode` 和 `schemaVersion` 必须分别与 bundle、彼此及本契约一致。`countryCode` 是两个大写字母的 ISO 3166-1 alpha-2 代码。

## 2. 通用值与枚举

`BasicCollectionJsonValue` 仅可为 `null`、布尔值、有限数值、字符串、上述值的数组，或键值同为该类型的对象。日期时间字段是有效的 ISO 8601 时间戳；`contentSha256` 是 SHA-256 十六进制摘要；URL 字段是有效的绝对 URL；`fieldPath` 是 canonical `market-overview` 字段的有效路径；所有引用的 `sourceId` 与 `factId` 必须在同一 bundle 中存在。
...
| 字段 | 类型与约束 |
|---|---|
| `sourceId` | 字符串；在 `sources` 内唯一 |
| `sourceName` | 字符串 |
| `sourceUrl` | 字符串，绝对 URL |
| `retrievedAt` | 字符串，ISO 8601 时间戳 |
| `publishedAt` | ISO 8601 时间戳或 `null` |
| `contentSha256` | 字符串，SHA-256 十六进制摘要 |
| `evidenceLocators` | 字符串数组 |
...
顶层字段为 `schemaVersion`（固定版本）、`runId`（字符串）、`countryCode`（字符串）和 `facts`（`BasicExtractedFact[]`）。每个事实包含 `factId`（唯一字符串）、`fieldPath`（有效 canonical 字段路径）、`status`（`BasicFactStatus`）、`evidence`（`BasicFactEvidence[]`）、`extractionMethod`（`BasicExtractionMethod`）和 `uncertainty`（字符串或 `null`）。

每个 `BasicFactEvidence` 包含 `sourceId`（引用已登记来源）、`locator`（字符串）、`rawValue`（`BasicCollectionJsonValue`）、`normalizedValue`（`BasicCollectionJsonValue`）、`unit`（字符串或 `null`）和 `year`（数值或 `null`）。
...
| `source` | 字符串；当 `sourceUrl` 为 `null` 时必须解释无链接原因 |
| `sourceUrl` | 字符串的绝对 URL 或 `null`；完整表示字段存在，而非必须有 URL |
| `collectedAt`、`updatedAt` | ISO 8601 时间戳字符串 |
| `credibility` | `Credibility` |
...
顶层字段为 `schemaVersion`（固定版本）、`runId`（字符串）、`countryCode`（字符串）、`status`（`ready-for-human-review` 或 `blocked`）、`missingFields`（有效 `fieldPath` 字符串数组）、`conflicts`（`BasicReviewConflict[]`）、`sourceChecks`（`BasicSourceCheck[]`）、`injectionRisks`（`BasicInjectionRisk[]`）、`publicationRecommendation`（`request-human-review` 或 `do-not-publish`）和 `humanDecision`（`BasicHumanDecision` 或 `null`）。没有自动 `publish` 推荐值。

`BasicReviewConflict` 为 `fieldPath`（有效路径）、`factIds`（至少一个已登记事实 ID）、`resolution`（`unresolved` 或 `resolved`）和 `notes`（字符串）。`BasicSourceCheck` 为 `sourceId`（已登记来源 ID）、`status`（`passed` 或 `failed`）和 `notes`（字符串或 `null`）。`BasicInjectionRisk` 为 `sourceId`（已登记来源 ID）、`locator`（字符串）、`severity`（`suspected` 或 `confirmed`）和 `details`（字符串）。`BasicHumanDecision` 为 `decision`（`approved` 或 `rejected`）、`reviewerId`（字符串）、`decidedAt`（ISO 8601 时间戳）和 `notes`（字符串）。
...
缺失、冲突或不可信的审计包只要结构正确，仍可 `valid`，但绝不可 `readyForHumanReview`。`missingFields` 非空或有 `status = missing` 的事实产生 `MISSING_REQUIRED_FACT`；任一 `resolution = unresolved` 的冲突产生 `UNRESOLVED_CONFLICT`；发现 discovery-only 材料、`UNVERIFIED` 可信度、`restricted` / `unknown` 访问状态、`suspected` / `confirmed` 注入风险、`injectionRisks`，或 `status = untrusted` 的事实时产生 `UNTRUSTED_INPUT`。
