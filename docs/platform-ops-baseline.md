...
- 结论：`100k`、`1m`、`10m` 三档只读场景均承载
- 适用范围：6 个已发布 BASIC 国家、国家列表/详情/模块只读 API、AI 占比 0、写入占比 0

## 1. 结论边界

本轮证明：在下述实测环境和固定请求矩阵下，一个 NestJS API 进程至少承载了 556 RPS，且三档均满足 M1 阈值。556 RPS 是“已验证点”，不是单实例极限；本轮没有继续加压寻找崩溃点。
...
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 100,000 | 6 / 6.000000 | 3,600 / 3,600 | 0 / 0 | 0 | 15.41 ms | 18.04 ms | 94.28% | 已承载 |
| 1,000,000 | 56 / 56.000000 | 33,600 / 33,600 | 0 / 0 | 0 | 3.81 ms | 4.35 ms | 99.24% | 已承载 |
| 10,000,000 | 556 / 555.998333 | 333,599 / 333,600 | 0 / 1 | 0 | 2.95 ms | 3.60 ms | 99.90% | 已承载 |

`10m` 的 1 个晚到请求在 600 秒窗口边界后完成，不是错误或丢弃；请求账目满足 `完成 + 晚到 + 丢弃 = 请求数`，达成率为 99.9997%。三个结果文件均包含 600 个单秒样本，API 指标区间之和分别约为 600 秒。

### 4.2 API 与数据库资源

| 每日请求 | CPU p95 | CPU p95 / 容量 | RSS p95 | RSS max | RSS max / 容量 | event-loop p99 | DB active / total 峰值 |
|---:|---:|---:|---:|---:|---:|---:|---:|
