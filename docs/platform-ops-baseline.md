...
- 基线日期：2026-07-19
- 实测代码：`65b58f0478c9690c0f85fd5cc4779b61d5154e7f`
- 有效运行：`task8-20260719-65b58f0-eventloop-window-p99`
- 结论：`100k`、`1m`、`10m` 三档只读场景均整体承载
- 适用范围：6 个已发布 BASIC 国家、国家列表/详情/模块只读 API、AI 占比 0、写入占比 0

## 1. 结论边界

本轮证明：在下述实测环境和固定请求矩阵下，一个 NestJS API 进程至少承载了 556 RPS，三档均满足 M1 吞吐、HTTP 延迟、错误率、缓存、资源、数据库连接和 event-loop 阈值。556 RPS 是“已验证点”，不是单实例极限；本轮没有继续加压寻找崩溃点。

固定矩阵只有 120 个请求键，实测 cache hit 为 94.19%–99.90%，高于容量模型的 80%。因此，本轮直接证明的是高缓存命中的 API 吞吐，不等于已经对 PostgreSQL 施加了模型中的 111.2 DB RPS；数据库规格不得按本轮低查询水位激进下调。
...
| 结果 schema | load result schema v2；每档 600 个连续有效的单秒 event-loop 窗口 |
...
| API dist manifest SHA-256 | `868827ffd019a44365c49808d46d45c1de0d5437a4953a3a41c5053e9d98ff10`；运行前后一致 |
...
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 100,000 | 6 / 6.000000 | 3,600 / 3,600 | 0 / 0 | 0 | 16.09 ms | 19.67 ms | 94.19% | 已承载 |
| 1,000,000 | 56 / 56.000000 | 33,600 / 33,600 | 0 / 0 | 0 | 3.71 ms | 4.37 ms | 99.23% | 已承载 |
| 10,000,000 | 556 / 555.996667 | 333,598 / 333,600 | 0 / 2 | 0 | 3.28 ms | 4.53 ms | 99.90% | 已承载 |

`10m` 的 2 个晚到请求在 600 秒窗口边界后完成，不是错误或丢弃；请求账目满足 `完成 + 晚到 + 丢弃 = 请求数`，达成率为 99.9994%。三个结果文件均为 schema v2，并各自包含 600 个单秒请求样本。

### 4.2 API 与数据库资源

| 每日请求 | CPU p95 | CPU p95 / 容量 | RSS p95 | RSS max | RSS max / 容量 | event-loop 汇总 p99 | DB 全程 active / total 峰值 |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 100,000 | 0.028190 cores | 0.0881% | 174.86 MiB | 175.74 MiB | 0.2748% | 23.461887 ms | 1 / 3 |
| 1,000,000 | 0.086498 cores | 0.2703% | 235.52 MiB | 235.52 MiB | 0.3683% | 23.609343 ms | 1 / 3 |
| 10,000,000 | 0.253566 cores | 0.7924% | 291.68 MiB | 291.68 MiB | 0.4562% | 24.920063 ms | 1 / 3 |

RSS 容量阈值按整个测量窗口的 `max` 判定，不用 p95 替代。DB 列重复显示的是整个顺序运行峰值，不冒充各场景精确分段值：active 峰值为 1、total 峰值为 3。采集器的 2,123 条墙钟时间戳中有 72 次负向跳变（最小 -1,763 ms），因此不能把墙钟值当作精确逐秒关联依据；行顺序和全程连接峰值仍有效。active 是每秒快照，也可能错过短查询，容量门槛以全程 total 连接峰值为主。

本轮 event-loop 口径为 `p99-of-one-second-window-p99`：后台固定 1 秒窗口采样，runner 只接受连续、递增、`valid=1` 的已完成窗口，再对 measurement 期 600 个单秒窗口 p99 计算汇总 p99。阈值以未舍入值严格判定 `< 50 ms`。

### 4.3 Event-loop 窗口与相位证据

| 场景 | anchor → baseline | 首次有效等待 | 同步等待 | measurement sequence | 有效 / 期望窗口 | 窗口时长合计 |
|---|---:|---:|---:|---:|---:|---:|
| `100k` | 6 → 7 | 43.003 ms | 900.721 ms | 128–727 | 600 / 600 | 600.168563 s |
| `1m` | 728 → 729 | 34.822 ms | 758.906 ms | 850–1,449 | 600 / 600 | 600.147848 s |
| `10m` | 1,450 → 1,451 | 37.719 ms | 701.174 ms | 1,572–2,171 | 600 / 600 | 600.122082 s |

三档都在有效 anchor 后等到精确的下一序列才固定负载起点；任意无效、重复超时、回退或跳号都会 fail closed。上表序列范围均恰好包含 600 个连续整数。
...
| 600 个单秒窗口 p99 的汇总 p99 < 50 ms | 通过 | 通过 | 通过 |
...
当前有效原始文件位于 `/home/kevin/navigator/artifacts/platform-ops/task8-20260719-65b58f0-eventloop-window-p99/`，被 `.gitignore` 排除，不提交到 Git。`manifest.sha256` 覆盖除自身外全部 9 个文件，清单自身 SHA-256 为 `fdef875c09dcce68eabc720e117139a116f335d6f49d370e573e47be5496f4fd`。

| Artifact | SHA-256 |
|---|---|
| `environment.json` | `5e812a93a470cfb8fd82612f4e89448de79d363e48dec863bbec7de076b9b459` |
| `api.log` | `ddf6c7097b28c5a1b777ea52116b41c05e243c10925a913b5d4b1a75a9601595` |
| `pg-connections.jsonl` | `2b8e8a1bea87b2fd7eebf71db6db508c1d35eddddabca61db4e72040df860661` |
| `100k.json` | `f277cb51543bae0b5bb03762a2b2eacda9bd3114ef47574dffd76dc580029ab1` |
| `1m.json` | `3e251b57ee756ff3b450fdf39bd364746621fcb8f6baf48223ad68786b3941ce` |
| `10m.json` | `9f7753d466ab1130752280178f5095e73dd500be51935c6f1bd81236b4ed0ae3` |
| `api-dist-before.sha256` | `868827ffd019a44365c49808d46d45c1de0d5437a4953a3a41c5053e9d98ff10` |
| `api-dist-after.sha256` | `868827ffd019a44365c49808d46d45c1de0d5437a4953a3a41c5053e9d98ff10` |
| `run-metadata.json` | `7f080f48d2e8e72b7e2c1abfcd5bc666727ae692f0bd5297b2d3aace5b7d7b43` |

独立完整性预检已确认：

- 444,961 行 API JSONL 全部可解析，5xx 行为 0；
- 2,123 行 PostgreSQL JSONL 全部可解析，字段和计数关系有效，active / total 峰值为 `1 / 3`；
- 三个结果 JSON 都为 schema v2，Git SHA、600 个连续有效 event-loop 窗口、状态计数、请求账目和测量时长一致；
- `manifest.sha256` 九项均通过 `sha256sum --check`，API dist 清单运行前后字节一致；
- artifact 中未发现 DATABASE_URL 或测试密码。

旧 `e46b33d4d4f6fbe638edbe9dd74c362012ccf68e` 运行已完整移至 `/home/kevin/navigator/artifacts/platform-ops/task8-20260719-e46b33d-superseded-eventloop-mean/`；其原 `manifest.sha256` 自身 SHA-256 仍为 `b358498956fca3522472743d376a8c9da9163b1bbceedc97913b13e71ab62878`。该运行只是 superseded 历史，不参与本报告的当前数字、阈值或整体承载判定。
