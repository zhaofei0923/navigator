...
- 人工确认：只有针对 r2 的独立人工发布决定获批后，才可创建和执行单独的 `DATA-BASIC-SA-PUBLISH` 任务。r1 永不发布，r2 也不授权 Standard、Complete 或 AI 启用。
