...
| P2 Web 展示 | 国家列表/详情/对比 + i18n | api-contract、coverage-levels、i18n | — |
| P3 AI 顾问 | RAG 管道 + 问答接口（占位边界） | ai-advisor | ⚠️ |
| P4 权限/会员/留资 | 门控 + 报告下载 + 留资 | auth-membership | ⚠️ |
| P5 Admin 后台 | 审核发布流 | api-contract、data-schema | — |
...
#### P2-2 国家列表 / 地图
- 目标：`GET /countries` + 列表/地图页，支持地区/标签/覆盖等级筛选。
- 验收：筛选生效；卡片显示覆盖等级徽章；E2E 通过。
- 人工确认：否。
...
#### P2-4 国家对比
- 目标：`GET /compare`（2–4 国）+ 对比页，缺失模块占位。
- 验收：`codes` 边界校验；对比矩阵正确；E2E 通过。
- 人工确认：否。
