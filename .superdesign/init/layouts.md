...
The application uses Next.js App Router. `RootLayout` supplies localization to every route, `DemoLayout` gates all authenticated demo routes, and `AppShell` renders the shared navigation, persistent synthetic-data notice, content frame, and footer. `ToolPageShell` and `IntelligenceListPage` are reusable route-level shells shared by the four workflow tools and five intelligence-list routes respectively.

## RootLayout

- File: `apps/web/app/layout.tsx`
- Description: Global document layout that resolves the request locale, supplies localized metadata, imports global CSS, and wraps every page with `LocaleProvider`.
...
- File: `apps/web/app/(demo)/layout.tsx`
- Description: Server layout for all private demo routes; rejects invalid shared-demo sessions and delegates visible framing to `AppShell`.
...
- Description: Shared authenticated application frame with brand, responsive primary navigation, language switcher, planning CTA, logout control, persistent demo warning, content region, and footer.
...
## ToolPageShell and ResultList

- File: `apps/web/components/tool-page-shell.tsx`
- Description: Shared route shell for the four workflow tools, plus the reusable result-list section rendered by controlled assistant, solar-storage, and feasibility outputs.
  ...
  return (
    <section className="tool-page">
      <Link className="back-link" href="/tools">
        ...
        {locale === "en" ? "Back to expansion tools" : "返回出海工具"}
      </Link>
...
- File: `apps/web/components/intelligence-list-page.tsx`
- Description: Shared filter, state, and responsive data-table shell used by the policy, risk, opportunity, tender, and partner list routes.
